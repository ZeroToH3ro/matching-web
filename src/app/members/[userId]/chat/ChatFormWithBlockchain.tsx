...
export default function ChatFormWithBlockchain({
  chatRoomId,
  chatAllowlistId,
  recipientAddress,
  recipientName,
  ...
  React.useEffect(() => {
    if (account?.address) {
      getProfileIdByAddress(client, account.address).then(profileId => {
  ...
  const sendOnChainMessage = async (messageText: string) => {
    ...
    try {
      ...
      const SERVER_OBJECT_IDS = process.env.NEXT_PUBLIC_SEAL_SERVER_IDS
        ? process.env.NEXT_PUBLIC_SEAL_SERVER_IDS.split(',').map(id =>
            id.trim()
          )
        : [
      ...
      if (
        ...
        chatRoomObj.data.content.dataType !== 'moveObject'
      ) {
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="w-full">
      <div className="flex items-center gap-1.5 md:gap-2">
        ...
        <Input
          fullWidth
          placeholder={
            hasOnChainChat ? 'Type encrypted message...' : 'Type a message'
          }
          variant="faded"
          {...register('text')}
